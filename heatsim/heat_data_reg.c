...
void solve(const long n, const double alpha, const double dx, const double dt, const double * restrict u, double * restrict u_tmp);
...
double solution(const double t, const double x, const double y, const double alpha, const double length) {
  ...
  double ret = exp(-2.0*alpha*PI*PI*t/(length*length)) * sin(PI*x/length) * sin(PI*y/length);
  //printf("t %lf x %lf y %lf sol %lf", t, x, y, ret);
  return ret;
}
